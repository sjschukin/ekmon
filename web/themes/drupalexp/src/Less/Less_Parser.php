...
class Less_Parser{
	...
	private function parseComment(){
		...
		if( $this->input[$this->pos+1] === '/' ){
			...
			return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Comment',array($match[0], true, $this->pos, $this->env->currentFileInfo));
		}

		//$comment = $this->MatchReg('/\\G\/\*(?:[^*]|\*+[^\/*])*\*+\/\n?/');
		$comment = $this->MatchReg('/\\G\/\*(?s).*?\*+\/\n?/');//not the same as less.js to prevent fatal errors
		if( $comment ){
			return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Comment',array($comment[0], false, $this->pos, $this->env->currentFileInfo));
		}
	}
	...
	private function parseEntitiesQuoted() {
		...
		if( $str ){
			...
			return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Quoted',array($str[0], $result, $e, $index, $this->env->currentFileInfo) );
		}
		return;
	}
	...
	private function parseEntitiesKeyword(){
		...
		if( $k ){
			...
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Keyword',$k);
		}
	}
	...
	private function FromKeyword( $keyword ){
		...
		if( Less_Colors::hasOwnProperty($keyword) ){
			// detect named color
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Color',substr(Less_Colors::color($keyword), 1));
		}

		if( $keyword === 'transparent' ){
			return $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Color', array( array(0, 0, 0), 0, true));
		}
	}
	...
	private function parseEntitiesCall(){
		...
		if ($name) {
			return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Call',array($name, $args, $index, $this->env->currentFileInfo) );
		}
	}
	...
	private function parseEntitiesAssignment() {
		...
		if( $value ){
			return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Assignment',array($key[0], $value));
		}
	}
	...
	private function parseEntitiesUrl(){
		...
		if( isset($value->value) || $value instanceof Less_Tree_Variable ){
			return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Url',array($value, $this->env->currentFileInfo));
		}

		return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Url', array( $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Anonymous',$value), $this->env->currentFileInfo) );
	}
	...
	private function parseEntitiesVariable(){
		...
		if ($this->PeekChar('@') && ($name = $this->MatchReg('/\\G@@?[\w-]+/'))) {
			return $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Variable', array( $name[0], $index, $this->env->currentFileInfo));
		}
	}
	...
	private function parseEntitiesVariableCurly() {
		...
		if( $this->input_len > ($this->pos+1) && $this->input[$this->pos] === '@' && ($curly = $this->MatchReg('/\\G@\{([\w-]+)\}/')) ){
			return $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Variable',array('@'.$curly[1], $index, $this->env->currentFileInfo));
		}
	}
	...
	private function parseEntitiesColor(){
		if ($this->PeekChar('#') && ($rgb = $this->MatchReg('/\\G#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})/'))) {
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Color',$rgb[1]);
		}
	}
	...
	private function parseEntitiesDimension(){
		...
		if( $value ){
			...
			if( isset($value[2]) ){
				return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Dimension', array($value[1],$value[2]));
			}
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Dimension',$value[1]);
		}
	}
	...
	function parseUnicodeDescriptor() {
		...
		if( $ud ){
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_UnicodeDescriptor', $ud[0]);
		}
	}
	...
	private function parseEntitiesJavascript(){
		...
		$str = $this->MatchReg('/\\G`([^`]*)`/');
		if( $str ){
			return $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Javascript', array($str[1], $this->pos, $e));
		}
	}
	...
	private function parseRulesetCall(){
		...
		if( $this->input[$this->pos] === '@' && ($name = $this->MatchReg('/\\G(@[\w-]+)\s*\(\s*\)\s*;/')) ){
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_RulesetCall', $name[1] );
		}
	}
	...
	function parseExtend($isRule = false){
		...
		do{
			...
			$extendList[] = $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Extend', array( $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Selector',$elements), $option, $index ));
	...
	private function parseMixinCall(){
		...
		if( $elements ){
			...
			if( $this->parseEnd() ){
				$this->forget();
				return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Mixin_Call', array( $elements, $args, $index, $this->env->currentFileInfo, $important));
			}
		}
	...
	private function parseMixinCallElements(){
		...
		while( true ){
			...
			$elements[] = $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Element', array($c, $e[0], $elemIndex, $this->env->currentFileInfo));
			$c = $this->MatchChar('>');
		}
		...
		while( true ){
			...
			if( $this->MatchChar(';') || $isSemiColonSeperated ){
				...
				if( count($expressions) > 1 ){
					$value = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Value', $expressions);
				}
	...
	private function parseMixinDefinition(){
		...
		if( $match ){
			...
			if( is_array($ruleset) ){
				$this->forget();
				return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Mixin_Definition', array( $name, $params, $ruleset, $cond, $variadic));
			}
	...
	private function parseAlpha(){
		...
		$this->expectChar(')');
		return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Alpha',$value);
	}
	...
	private function parseElement(){
		...
		if( is_null($e) ){
			...
			if( $this->MatchChar('(') ){
				if( ($v = $this->parseSelector()) && $this->MatchChar(')') ){
					$e = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Paren',$v);
					$this->forget();
				}else{
					$this->restore();
				}
		...
		if( !is_null($e) ){
			return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Element',array( $c, $e, $index, $this->env->currentFileInfo));
		}
	}
	...
	private function parseSelector( $isLess = false ){
		...
		if( $elements ){
			return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Selector',array($elements, $extendList, $condition, $index, $this->env->currentFileInfo));
		}
		if( $extendList ) {
	...
	private function parseAttribute(){
		...
		return $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Attribute',array( $key, $op[0], $val));
	}
	...
	private function parseBlockRuleset(){
		...
		if( $block ){
			$block = $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Ruleset',array( null, $block));
		}
	...
	private function parseDetachedRuleset(){
		...
		if( $blockRuleset ){
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_DetachedRuleset',$blockRuleset);
		}
	}
	...
	private function parseRuleset(){
		...
		if( $selectors ){
			...
			if( is_array($rules) ){
				$this->forget();
				return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Ruleset',array( $selectors, $rules)); //Less_Environment::$strictImports
			}
		}
	...
	private function parseNameValue(){
		...
		if( $match ){
			...
			return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_NameValue',array( $match[1], $match[2], $index, $this->env->currentFileInfo));
		}
	...
	private function parseRule( $tryAnonymous = null ){
		...
		if( $name ){
			...
			if( $value && $this->parseEnd() ){
				$this->forget();
				return $this->NewObj6('\Drupal\drupalexp\Less\Less_Tree_Rule',array( $name, $value, $important, $merge, $startOfRule, $this->env->currentFileInfo));
			}else{
				$this->furthest = $this->pos;
				$this->restore();
	...
	function parseAnonymousValue(){
		...
		if( preg_match('/\\G([^@+\/\'"*`(;{}-]*);/',$this->input, $match, 0, $this->pos) ){
			...
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Anonymous',$match[1]);
		}
	}
	...
	private function parseImport(){
		...
		if( $dir ){
			...
			if( $path ){
				...
				if( $this->MatchChar(';') ){
					if( $features ){
						$features = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Value',$features);
					}

					$this->forget();
					return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Import',array( $path, $features, $options, $this->pos, $this->env->currentFileInfo));
				}
			}
		}
	...
	private function parseMediaFeature() {
		...
		do{
			...
			if( $e ){
				...
				if ($this->MatchChar(')')) {
					if ($p && $e) {
						$r = $this->NewObj7('\Drupal\drupalexp\Less\Less_Tree_Rule', array( $p, $e, null, null, $this->pos, $this->env->currentFileInfo, true));
						$nodes[] = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Paren',$r);
					} elseif ($e) {
						$nodes[] = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Paren',$e);
					} else {
						return null;
					}
				} else
		...
		if ($nodes) {
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Expression',$nodes);
		}
	}
	...
	private function parseMedia() {
		if( $this->MatchReg('/\\G@media/') ){
			...
			if( is_array($rules) ){
				return $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Media',array( $rules, $features, $this->pos, $this->env->currentFileInfo));
			}
		}
	}
	...
	private function parseDirective(){
		...
		if( $hasIdentifier ){
			...
			if( $value ){
				$value = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Anonymous',trim($value[0]));
			}
		}
		...
		if( $rules || (!$hasBlock && $value && $this->MatchChar(';'))) {
			$this->forget();
			return $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Directive',array($name, $value, $rules, $index, $this->env->currentFileInfo));
		}
	...
	private function parseValue(){
		...
		if( $expressions ){
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Value',$expressions);
		}
	}
	...
	private function parseSub (){
		...
		if( $this->MatchChar('(') ){
			...
			if( $a ){
				$this->expectChar(')');
				return $this->NewObj2('\Drupal\drupalexp\Less\Less_Tree_Expression',array( array($a), true) ); //instead of $e->parens = true so the value is cached
			}
		}
	}
	...
	function parseMultiplication(){
		...
		if( $return ){
			while( true ){
				...
				$m->parensInOp = true;
				$a->parensInOp = true;
				$return = $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Operation',array( $op, array( $return, $a ), $isSpaced) );
			}
		}
		return $return;
	...
	private function parseConditions() {
		...
		if( $a ){
			while( true ){
				...
				$return = $this->NewObj4('\Drupal\drupalexp\Less\Less_Tree_Condition',array('or', $return, $b, $index));
			}
			return $return;
		}
	}
	...
	private function parseCondition() {
		...
		if( $a ){
			...
			if( $op ){
				...
				if( $b ){
					$c = $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Condition',array($op[0], $a, $b, $index, $negate));
				} else {
					$this->Error('Unexpected expression');
				}
			} else {
				$k = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Keyword','true');
				$c = $this->NewObj5('\Drupal\drupalexp\Less\Less_Tree_Condition',array('=', $a, $k, $index, $negate));
			}
			$this->expectChar(')');
			return $this->MatchReg('/\\Gand/') ? $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Condition',array('and', $c, $this->parseCondition())) : $c;
		}
	}
	...
	private function parseOperand (){
		...
		if( $negate ){
			$o->parensInOp = true;
			$o = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Negative',$o);
		}
	...
	private function parseExpression (){
		...
		do{
			...
			if( $e ){
				...
				if( !$this->PeekReg('/\\G\/[\/*]/') ){
					...
					if( $delim ){
						$entities[] = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Anonymous',$delim);
					}
				}
			}
		}while($e);
		...
		if( $entities ){
			return $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Expression',$entities);
		}
	}
	...
	private function parseRuleProperty(){
		...
		if( (count($name) > 1) && $this->rulePropertyMatch('/\\G\s*((?:\+_|\+)?)\s*:/', $offset, $length, $index, $name) ){
			...
			foreach($name as $k => $s ){
				if( !$s || $s[0] !== '@' ){
					$name[$k] = $this->NewObj1('\Drupal\drupalexp\Less\Less_Tree_Keyword',$s);
				}else{
					$name[$k] = $this->NewObj3('\Drupal\drupalexp\Less\Less_Tree_Variable',array('@' . substr($s,2,-1), $index[$k], $this->env->currentFileInfo));
				}
			}
			return $name;
		}
	...
	public function ArgCache($args){
		return implode(',',array_map( array('\Drupal\drupalexp\Less\Less_Parser','ArgString'),$args));
	}
